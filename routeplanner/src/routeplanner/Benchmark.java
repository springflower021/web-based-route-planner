...
public class Benchmark {
	...
	public static void main(String[] args) {
		...
		Node closestNode;
		try { 
			closestNode = graph.findNearestNode(lat, lon);
		} catch (Exception e) {
			e.printStackTrace();
			return;
		}
		coords[0] = closestNode.getLati();
	    coords[1] = closestNode.getLongi();
		...
		try (BufferedReader bufferedReader = new BufferedReader(new FileReader(quePath))) {
			...
			while ((currLine = bufferedReader.readLine()) != null) {
				...
				Dijkstra oneToOneDijkstra = new Dijkstra(graph);
				Node source = graph.getNode(oneToOneSourceNodeId);
				Node target = graph.getNode(oneToOneTargetNodeId);
				oneToOneDijkstra.executeOneToOne(source, target);
				oneToOneDistance = oneToOneDijkstra.getminimalDistance(target);
		...
		Dijkstra oneToAllDijkstra = new Dijkstra(graph);
		Node source = graph.getNode(sourceNodeId);
		oneToAllDijkstra.executeOneToAll(source);
		...
		oneToAllDistance = oneToAllDijkstra.getminimalDistance(target);
